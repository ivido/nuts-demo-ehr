<template>
  <div>
    ...
    <div class="mt-6">
      <label>Status</label>
      <div v-if="episode" class="bg-white p-5 shadow-sm rounded-lg mb-3">
        {{ episode.status }}
      </div>
    </div>
    ...
    <div class="mt-8">
      <div class="flex justify-between items-center mb-4">
        ...
        <button
          class="float-right inline-flex items-center bg-nuts w-10 h-10 rounded-lg justify-center shadow-md"
          @click="$router.push({ name: 'ehr.patient.episode.newPrescription' })"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="24px"
      ...
      <div
        ...
        class="bg-white p-5 shadow-sm rounded-lg mb-3"
      >
        <table class="prescriptions-list min-w-full divide-y divide-gray-200">
          <thead>
            <tr>
...
export default {
  ...
  methods: {
    ...
    fetchReports(patientID, episodeID) {
      this.$api
        .getReports({ patientID, episodeID })
        .then((reports) => (this.reports = reports))
        .catch((e) => this.$status.error(e));
    },
    fetchPrescriptions(patientID, episodeID) {
      this.$api
        .getPrescriptions({ patientID, episodeID })
        .then((prescriptions) => (this.prescriptions = prescriptions))
        .catch((e) => this.$status.error(e));
    },
    fetchCollaborations(episodeID) {
      this.$api
        .getCollaboration({ episodeID })
    ...
    chooseCollaboration(collaboration) {
      ...
      this.$api
        .createCollaboration({
          episodeID,
          body: { sender: { did: collaboration.did } },
        })
    ...
    searchOrganizations(query) {
      this.$api
        .searchOrganizations({
          query: query,
          didServiceType: "zorginzage-demo",
        })
        .then((organizations) => {
