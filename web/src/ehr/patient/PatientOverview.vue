<template>
  <div class="mt-10 bg-white px-7 py-5 rounded-lg shadow-sm flex flex-col">
    ...
    <div class="dossier-list">
      ...
      >
        ...
        <tbody>
          ...
          >
            ...
            <td>
              {{
                dossier.transfer && dossier.transfer.negotiations
                  ? dossier.transfer.negotiations
                      .map((n) => n.organization.name)
                      .join(", ")
                  : ""
              }}
            </td>
      ...
      <div v-else-if="loadingDossiers">Loading...</div>

      <div v-else class="min-w-full">
        No dossiers for this patient found.
        <router-link :to="{ name: 'ehr.patient.dossier.new' }"
  ...
  <div class="bg-white px-7 py-5 rounded-lg shadow-sm mt-8">
    ...
    <table class="reports-list min-w-full divide-y divide-gray-200">
      ...
      <tbody>
        <tr v-for="prescription in prescriptions" v-bind:key="prescription.id">
          <td>{{ prescription.medicationName }}</td>
          <td>{{ prescription.source }}</td>
          <td>{{ prescription.episodeName }}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  computed: {
    episodeDossiers() {
      return this.dossiers.map((dossier) => {
        dossier.transfer = this.transfers.find(
          (transfer) => transfer.dossierID === dossier.id
        );
        return dossier;
      });
    },
  ...
  methods: {
    ...
    fetchDossiers() {
      ...
      this.$api
        .getDossier({ patientID: this.$route.params.id })
        .then((dossiers) => {
    ...
    fetchReports() {
      this.$api
        .getReports({ patientID: this.$route.params.id })
        .then((reports) => (this.reports = reports))
        .catch((error) => this.$status.error(error));
    },
    fetchPrescriptions() {
      this.$api
        .getPrescriptions({ patientID: this.$route.params.id })
        .then((prescriptions) => (this.prescriptions = prescriptions))
        .catch((error) => this.$status.error(error));
    },
    fetchTransfers() {
      this.$api
        .getPatientTransfers({ patientID: this.$route.params.id })
        .then((transfers) => {
          this.transfers = transfers;
          // Also fetch negotiations so we can show the "network" of the dossier
          return Promise.all(
            this.transfers.map((t) =>
              this.$api.listTransferNegotiations({ transferID: t.id })
            )
          );
        })
        .then((negotiations) => {
    ...
    openDossier(dossier) {
      ...
      if (dossier.transfer) {
        this.$router.push({
          ...
          params: { id: patientID, transferID: dossier.transfer.id },
        });
      } else {
        this.$router.push({
          name: "ehr.patient.episode.edit",
          params: { id: patientID, episodeID: dossier.id },
        });
      }
    },
  },
